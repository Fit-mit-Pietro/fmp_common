...
class BodySelectorViewService {
  ...
  static const List<Map<String, dynamic>> symptomsData = [
    ...
    {
      "id": "head_neck_tension",
      "label": "Spannungs \nKopfschmerzen /\ndurch Nacken\nVerspannungen,"
    },
    {
      "id": "austrahlung_beine_gesaes",
      "label": "Ausstrahlung und\nKribbeln in den\nBeinen oder\nund ins Gesäß"
    },
    ...
    {
      "id": "verkürzung_nackenmuskulatur",
      "label":
          "Verkürzung Nacken-\nmuskulatur und\nTrapezius\n(Tastaturhaltung)"
    },
  ];
  ...
  List<BodyPartSymptom> getSymptomsByKeys(List<String> keys) {
    ...
    for (var key in keys) {
      BodyPartSymptom? symptom =
          availableSymptoms.firstWhereOrNull((element) => element.id == key);
      if (symptom != null) {
  ...
  Future<BodySelectorViewModel> loadModel() async {
    ...
    List<BodyPart> frontParts = await getBodyPartsFromSvg(
        "assets/img/body_map/body_front_svg.svg", availableBodyParts);
    List<BodyPart> backParts = await getBodyPartsFromSvg(
        "assets/img/body_map/body_back_svg.svg", availableBodyParts);
  ...
  Future<List<BodyPart>> getBodyPartsFromSvg(
    ...
    List<BodyPart> parts = [];
    Map<String, String> frontPartsPaths =
        await loadBodyPartsFromSvg(svgImage: path);
  ...
  Future<Map<String, String>> loadBodyPartsFromSvg(
    ...
    for (var element in paths) {
      ...
      // TODO Clarify this line @Pietro ? Is there a naming convention with the bodymap svg files?
